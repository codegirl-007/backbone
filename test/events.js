(function(QUnit) {
  ...
  QUnit.test('on and trigger', function(assert) {
    assert.expect(2);
    var obj = {counter: 0};
    Object.assign(obj, Backbone.Events);
  ...
  QUnit.test('listenTo and stopListening', function(assert) {
    assert.expect(1);
    var a = Object.assign({}, Backbone.Events);
    var b = Object.assign({}, Backbone.Events);
  ...
  QUnit.test('listenTo yourself', function(assert) {
    assert.expect(1);
    var e = Object.assign({}, Backbone.Events);
  ...
  QUnit.test('stopListening cleans up references', function(assert) {
    ...
    assert.equal(Object.keys(a._listeningTo || {}).length, 0);
    assert.equal(Object.keys(b._events.event).length, 1);
    assert.equal(Object.keys(b._listeners || {}).length, 0);
  ...
  QUnit.test('listenToOnce with event maps cleans up references', function(assert) {
    ...
    b.trigger('one');
    assert.equal(Object.keys(a._listeningTo).length, 1);
  });
  ...
  QUnit.test('bind a callback with a supplied context', function(assert) {
    ...
    var obj = Object.assign({}, Backbone.Events);
  ...
  QUnit.test('callback list is not altered during trigger', function(assert) {
    assert.expect(2);
    var counter = 0, obj = Object.assign({}, Backbone.Events);
  ...
  QUnit.test('once variant one', function(assert) {
    ...
    var a = Object.assign({}, Backbone.Events).once('event', f);
    var b = Object.assign({}, Backbone.Events).on('event', f);
  ...
  QUnit.test('once with asynchronous events', function(assert) {
    ...
    assert.expect(1);
    // Inline debounce for this test
    var timeout;
    var func = function() {
      var context = this, args = arguments;
      clearTimeout(timeout);
      timeout = setTimeout(function() { assert.ok(true); done(); }, 50);
    };
    var obj = Object.assign({}, Backbone.Events).once('async', func);
  ...
  QUnit.test('once without a callback is a noop', function(assert) {
    assert.expect(0);
    Object.assign({}, Backbone.Events).once('event').trigger('event');
  });
  ...
  QUnit.test('event functions are chainable', function(assert) {
    var obj = Object.assign({}, Backbone.Events);
    var obj2 = Object.assign({}, Backbone.Events);
    var fn = function() {};
  ...
  QUnit.test('#3448 - listenToOnce with space-separated events', function(assert) {
    assert.expect(2);
    var one = Object.assign({}, Backbone.Events);
    var two = Object.assign({}, Backbone.Events);
    var count = 1;
  ...
  QUnit.test('#3611 - stopListening is compatible with non-Backbone event libraries', function(assert) {
    ...
    other.trigger('test');
    assert.equal(Object.keys(obj._listeningTo || {}).length, 0);
  });
})(QUnit);
